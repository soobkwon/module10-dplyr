...
amt.time.gain <- mutate(flights, gain = arr_delay - dep_delay)
...
sort.amt.time.gain <- arrange(amt.time.gain, desc(gain))

# For practice, repeat the last 2 steps in a single statement using the pipe operator
sorted.time.gain <- mutate(flights, gain = arr_delay - dep_delay) %>% 
  arrange(desc(gain))
